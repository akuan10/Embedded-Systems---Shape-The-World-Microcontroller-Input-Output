...
//		do {
//		In = GPIO_PORTF_DATA_R & 0x10; // Retrieve the value of PF4		
//		} while(In == 0x10);
...
void PortF_Init(void){ volatile unsigned long delay;
  ...
  GPIO_PORTF_DIR_R &= ~0x10;         // 4.1) PF4 input, \ 1110,1111 => PF4 = 0
  GPIO_PORTF_DIR_R |= 0x0E;          // 4.2) PF3,2,1 output  \ 0000,1110 => PF1,2,3 = 1
  GPIO_PORTF_AFSEL_R &= 0x00;        // 5) no alternate function
  GPIO_PORTF_PUR_R |= 0x10;          // 6) enable pullup resistor on PF4  \ 0001,0000     
  GPIO_PORTF_DEN_R |= 0x1E;          // 7) enable digital pins PF4-PF1 \ 0001,1110
}
// Color    LED(s) PortF
// dark     ---    0
// red      R--    0x02
...
void WaitForASLow(void)
{
	...
	}while(In == 0x10); //PF4 not press, loop
}
...
void WaitForASHigh(void)
{
	...
	do{
		In = GPIO_PORTF_DATA_R & 0x10;
	}while(In != 0x10); //PF4 is pressed, loop
